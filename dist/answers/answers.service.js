...
let AnswersService = class AnswersService {
    ...
    async getAnswersDiaryByDate({ userId, date, limit, direction, }) {
        const whereDate = direction === 0 ? typeorm_2.LessThan(date) : typeorm_2.MoreThan(date);
    ...
    hasSixParsAndNotToday(answers) {
        return answers.length === 6 && answers[5] && answers[5].date !== date_1.getDateString({});
    }
